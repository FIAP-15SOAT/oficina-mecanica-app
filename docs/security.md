...
- **Troca ou reset de senha não revogam sessões/tokens já emitidos.** Não há lista de revogação (blacklist) de JWT — nem interno, nem externo. A mitigação é a recarga de `isActive` do usuário e do vínculo `UserCustomer` a cada requisição: uma conta desativada ou um vínculo removido deixa de autorizar operações no próximo request, mesmo que o token continue criptograficamente válido até expirar.
...
**A URL nunca é registrada em forma bruta.** O mecanismo foi desenhado quando `GET /api/quotes/:id/decisions?token=<JWT assinado>` era a única rota `@Public()` — o token na query autorizava aprovar ou rejeitar um orçamento, e registrar `req.url` gravaria uma credencial viva num armazenamento de logs de terceiros, justamente quando um `4xx` nessa rota tornaria o log mais desejável. Essa rota foi removida (ver [ADR 0004](./adr/0004-autenticacao-de-clientes.md) — a decisão de orçamento agora exige o Cliente da Oficina autenticado via `POST /api/me/quotes/:id/decisions`), e a única rota `@Public()` remanescente (`POST /api/auth/password-reset-confirmations`) não carrega segredo na URL. A defesa permanece ativa por princípio, não por necessidade pontual: qualquer query string continua passando pela mesma classificação do corpo (um campo `token` seria SEGREDO → removido) e é emitida como `url.query`, uma string já sanitizada.
