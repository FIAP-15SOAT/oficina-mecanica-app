...
**A URL nunca é registrada em forma bruta.** `GET /api/quotes/:id/decisions?token=<JWT assinado>` é a única rota `@Public()`, e o token autoriza aprovar ou rejeitar um orçamento — registrar `req.url` gravaria uma credencial viva num armazenamento de logs de terceiros, e um `4xx` nessa rota é exatamente quando alguém iria querer o log. A query passa pela mesma classificação do corpo (o campo `token` é SEGREDO → removido) e é emitida como `url.query`, uma string já sanitizada.
