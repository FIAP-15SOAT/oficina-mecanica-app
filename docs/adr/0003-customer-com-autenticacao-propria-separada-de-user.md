# ADR 0003: `Customer` ganha autenticação própria, separada de `User`
...
- `docs/adr/0002-document-value-object-compartilhado-entre-user-e-customer.md` — decisão anterior relacionada (generalização do VO `Document`), que estabeleceu o precedente de generalizar o que é genuinamente compartilhado (validação de documento) sem misturar o que não é (identidade de staff vs. cliente).
