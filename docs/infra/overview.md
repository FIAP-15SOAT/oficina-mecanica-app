...
| 5 | **`oficina-mecanica-gateway`** | Terraform | **AWS API Gateway (HTTP API)**, stage, VPC Link, security group e log de acesso — a **API Gateway** da solução | S3 `infra/prod-simulated/gateway/terraform.tfstate` |
...
Os **cinco** stacks Terraform têm states separados porque o stack de Kubernetes (Camada 2) e o de banco (Camada 3) consomem a rede e as subnets da Camada 1, o stack do API Gateway (Camada 5) consome a rede da Camada 1 e o caminho privado da Camada 2, e o stack da função (Camada 6) consome a rede da Camada 1, o endereço e a credencial da Camada 3 e o `api_execution_arn` da Camada 5 — todos via `data.terraform_remote_state`. A camada 4 não é Terraform: são manifests declarativos aplicados pelo pipeline de CD, porque imagem, envs e escala mudam com muito mais frequência que a plataforma. Detalhes da divisão em [kubernetes.md › Motivo da divisão](kubernetes.md#motivo-da-divisão) e [terraform.md](terraform.md).
...
Há **dois** caminhos de entrada: o público, pelo API Gateway, e o de diagnóstico, por túnel. A partir do pod, tudo é comunicação **intra-cluster** por DNS de Service:
...
4. **`oficina-mecanica-gateway`**: consome a VPC e as subnets privadas do estado de rede e o `api_nlb_listener_arn` do estado de Kubernetes, provisionando o API Gateway, o VPC Link e o log de acesso do API Gateway. É o **último** da ordem, porque a integração privada precisa do listener já existente — o stack falha com mensagem explícita, por precondição, se ele não estiver publicado.

> **Ordem de aplicação desta mudança.** `oficina-mecanica-k8s` (NLB) → `oficina-mecanica-app` (este `Service` como `NodePort`) → `oficina-mecanica-gateway` (API Gateway). O rollback é a ordem inversa, e ela importa: reverter o `Service` antes de remover o API Gateway deixa o target group sem destino.
...
- 🌐 [`oficina-mecanica-gateway`](https://github.com/FIAP-15SOAT/oficina-mecanica-gateway) — o API Gateway: rotas, contrato OpenAPI, integração privada, observabilidade e ADRs do API Gateway.
