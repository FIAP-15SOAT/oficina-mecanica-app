...
export async function seedWorkOrderStatusDefs(prisma: PrismaClient): Promise<void> {
  ...
  for (const row of rows) {
    await prisma.workOrderStatusDef.upsert({
      where: { code: row.code },
  ...
  console.log(`✅ WorkOrderStatusDef: ${rows.length} rows seeded`);
}
