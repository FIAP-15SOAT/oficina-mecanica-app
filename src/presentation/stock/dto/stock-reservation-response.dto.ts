...
export class StockReservationWorkOrderDto {
  ...
  @ApiPropertyOptional({ type: StockReservationWorkOrderCustomerDto, description: 'Dados do Cliente', nullable: true })
  customer: StockReservationWorkOrderCustomerDto | null;

  @ApiPropertyOptional({ type: StockReservationWorkOrderVehicleDto, description: 'Dados do Veículo', nullable: true })
  vehicle: StockReservationWorkOrderVehicleDto | null;

  @ApiPropertyOptional({ type: StockReservationWorkOrderAssignedUserDto, description: 'Mecânico responsável', nullable: true })
  assignedUser: StockReservationWorkOrderAssignedUserDto | null;
}

export class StockReservationResponseDto {
  @ApiProperty({ description: 'ID único da Reserva de Estoque', example: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890', format: 'uuid' })
  id: string;
  ...
  @ApiProperty({ type: StockReservationWorkOrderDto, description: 'Ordem de Serviço que originou a reserva' })
  workOrder: StockReservationWorkOrderDto;

  @ApiProperty({ description: 'Quantidade reservada', example: 2 })
  quantity: number;

  @ApiProperty({ description: 'Data/hora da criação da reserva', example: '2026-04-21T10:30:00.000Z', format: 'date-time' })
  createdAt: Date;
}
